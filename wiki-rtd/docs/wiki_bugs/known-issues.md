...
## Big issues
- `Duckstation` memory cards folders are not created  making memory cards not work (default is for PSX is RetroArch - Swanstation). The path is:`/retrodeck/saves/psx/duckstation/memcards/` this can problem can be solved by manually create the folders.
